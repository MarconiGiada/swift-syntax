...
for (food,value) in polarizingFoods {
    if value {
        print(food + ", yum!")
    } else {
        print(food + ", yuck!")
...
for (key,value) in rapperPhoneNumbers {
    if value.containsString("415") {
        rapperPhoneNumbers[key] = rapperPhoneNumbers[key]?.stringByReplacingOccurrencesOfString("415", withString: "510")
    }
}
